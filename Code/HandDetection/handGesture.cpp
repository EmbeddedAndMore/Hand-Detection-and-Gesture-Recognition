...
void HandGesture::printGestureInfo(Mat src){
	...
	int xpos=src.cols/1.4;
	int ypos = 10; //src.rows/3;
	float fontSize=0.9f;
	int lineChange=14;
	string info= "Gesture info:";
	//putText(src,info,Point(ypos,xpos),fontFace,fontSize,fColor);
	//xpos+=lineChange;
	info=string("Number of defects(fingers): ") + string(intToString(nrOfDefects)) ;
	putText(src,info,Point(ypos,xpos),fontFace,fontSize  ,fColor);
	/*xpos+=lineChange;
...
void HandGesture::eleminateDefects(MyImage *m){
	...
	for (const Vec4i& v : defects[cIdx])
	{
		float depth = v[3] / 256;
...
void HandGesture::drawFingerTips(MyImage *m){
	...
	for(int i=0;i<fingerTips.size();i++){
		...
		circle(m->src, p, 10, Scalar(173, 244, 66), 4);
   	 }
}
