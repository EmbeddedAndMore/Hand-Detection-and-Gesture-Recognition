...
void printText(Mat src, string text){
	int fontFace = FONT_HERSHEY_PLAIN;
	putText(src, text, Point(src.cols / 1.2, src.rows / 3), fontFace, 2.5f, Scalar(66, 125, 244), 2);
}
...
void waitForPalmCover(MyImage* m){
	...
	for(int i =0;i<50;i++){
		...
		//}
		rectangle(m->src, Point(m->src.cols / 4, m->src.rows / 6), Point(m->src.cols / 2 + square_len, m->src.rows / 1.5 + square_len), Scalar(244, 134, 66), 2);
...
void myDrawContours(MyImage *m,HandGesture *hg){
	drawContours(m->src, hg->hullP, hg->cIdx, cv::Scalar(66, 134, 244), 2, 8, vector<Vec4i>(), 0, Point());
...
//		approxPolyDP( Mat(hg->contours[hg->cIdx]), hg->contours[hg->cIdx], 11, true );
		...
		hg->ControueArea = contourArea(hg->hullP[hg->cIdx]);
...
int main(){
	...
	for(;;){
		...
		//=========== Tracking
		tracker.addBoundingRect(hg.bRect);
		Point CurrentPosition = tracker.getCurrentPosition();
		Point NextPosition = tracker.predictNextPosition();

		circle(m.src, CurrentPosition, 4, Scalar(255, 255, 255), 2);
		circle(m.src, NextPosition, 4, Scalar(0, 0, 0), 2);

		double handAngle = 0;
		double Major_Axis = 0;
		double Minor_Axis = 0;
		if (hg.hullP[hg.cIdx].size() > 3)
		{
			
			RotatedRect fit_elipce = fitEllipse(hg.hullP[hg.cIdx]);

			Major_Axis = fit_elipce.size.width / 2;
			Minor_Axis = fit_elipce.size.height / 2;
			fit_elipce.angle += 5;
			handAngle = fit_elipce.angle;
			ellipse(m.src, fit_elipce, Scalar(0, 0, 255), 2, 8);
		}
		//=============================================================
		...
		//=========== gesture detection
		double aspectRatio = hg.bRect_height / hg.bRect_width;


		cout << "Area : " << hg.ControueArea << "	" << "aspectRatio : " << aspectRatio << "	" << "Angle : " << handAngle << endl;


		if (hg.ControueArea < 50000 && hg.nrOfDefects < 2)
		{
			if (aspectRatio > 1.9 && aspectRatio < 2.5)
			{
				printText(m.src, "back");
			}
			else if (aspectRatio < 1.5)
				printText(m.src, "Stop");
		}
		else if (hg.ControueArea < 70000 && hg.ControueArea > 50000 && hg.nrOfDefects < 2)
		{
			if (handAngle < 15 || handAngle > 165)
			{
				printText(m.src, "Move");
			}
			else if (handAngle < 165 && handAngle > 45)
			{
				if (hg.ControueArea < 70000 && hg.ControueArea > 50000 && hg.nrOfDefects < 2)
				{
					printText(m.src, "Left");
				}
			}
			else if (handAngle > 15 && handAngle < 45)
			{
				if (hg.ControueArea < 70000 && hg.ControueArea > 50000 && hg.nrOfDefects < 2)
				{
					printText(m.src, "Right");
				}
			}
		}
		else if (hg.ControueArea > 70000 && hg.nrOfDefects < 2)
		{
			printText(m.src, "Front");
		}
		else if (hg.nrOfDefects >= 4)
		{
			printText(m.src, "Start");
		}
